...
from zone_generation_dbkm_debug import *
...
zone_list_cur_choice_save_in=zones_all_choices(demand_limit,ntn_df_avg, max_multual_dist, across_branch_nodes, area_non_walk_df, G, depot_point)
final_decision_zone_list_dbkm=get_final_decision_zone_list(zone_list_cur_choice_save_in)
...
final_decision_zone_list=final_decision_zone_list_dbkm
all_decision_list_result=zone_list_cur_choice_save_in
...
def test_node_group_cost():
    ...
    for i in range(len(final_decision_zone_list)):
        ...
        filename = "cost_cs_hanover_1size_dbkm.csv"
        cur_df.to_csv(filename)   
test_node_group_cost() 
    
    
cs_cost_read=pd.read_csv ('cost_cs_hanover_1size_dbkm.csv')
...
for i in range(1,13):
    ...
    df_obj.to_csv("evaluation_costs_1size_dbkm_"+str(demand_limit)+".csv")

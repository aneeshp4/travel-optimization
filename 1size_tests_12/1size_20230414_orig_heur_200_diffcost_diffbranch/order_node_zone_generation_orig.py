...
def order_node_and_zone_generation_one_branch(max_zone_dem_input,max_multual_dist, ntn_df_avg, branch_all_nodes_input, area_non_walk_df, depot_node, G, longest_distance_from_depot):
    ...
    max_zone_dem=max_zone_dem_input
    min_zone_dem=1
...
def check_zone_demand(this_zone_nodes,area_non_walk_df, max_zone_dem):
    cur_total_demand=0
    for i in range(len(this_zone_nodes)):
        cur_total_demand=cur_total_demand+area_non_walk_df['pad_nodes_all_pickup_amount'][this_zone_nodes[i]]+area_non_walk_df['pad_nodes_all_dropoff_amount'][this_zone_nodes[i]]
    
    if cur_total_demand>max_zone_dem and len(this_zone_nodes)>1:
        over=1
    else:
        over=0
...
def combine_all_zones(max_zone_dem_input,max_multual_dist, ntn_df_avg, across_branch_nodes_input, start_num_branch, area_non_walk_df, depot_node, G, longest_distance_from_depot):
    ...
    for i in range(len(across_branch_nodes_input)):
        ...
        node_order_list_decision, node_distance_order_list_decision, all_sz_decision, all_sz_demand_decision=order_node_and_zone_generation_one_branch(max_zone_dem_input,max_multual_dist, ntn_df_avg, cur_decision_branch, area_non_walk_df, depot_node, G, longest_distance_from_depot)
        ...
        for j in range(len(decision_list)):
            ...
            over_check=check_zone_demand(cur_zone,area_non_walk_df, max_zone_dem_input)
            if (exceed_check==0 and over_check==0) or len(cur_zone)==1:

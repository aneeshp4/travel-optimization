...
from zone_generation_exact import *
from  order_node_zone_generation import *
import warnings
...
#generate_node_to_node_distances(area_non_walk_df, depot_point,outer_bound_dist, not_reachable_dist)
ntn_df_avg=get_ntn_df_avg(not_reachable_dist, compensate_dist)
zones_all_decisions_out=zone_generation_process(max_multual_dist, ntn_df_avg, demand_limit, across_branch_nodes, area_non_walk_df, depot_node, G, longest_distance_from_depot)
final_decision_zone_list=get_final_decision_zone_list(zones_all_decisions_out)
all_node_order_result=get_combine_node_order(across_branch_nodes, initial_num_branch, area_non_walk_df, depot_node, G, longest_distance_from_depot)
all_decision_list_result=zones_all_decisions_out
...
def test_node_group_cost():
    ...
    for i in range(len(final_decision_zone_list)):
        ...
        filename = "cost_cs_hanover_1size_plus.csv"
        cur_df.to_csv(filename)   
test_node_group_cost() 
    
    
cs_cost_read=pd.read_csv ('cost_cs_hanover_1size_plus.csv')
...
for i in range(1,13):
    ...
    df_obj.to_csv("evaluation_costs_1size_plus_"+str(demand_limit)+".csv")
